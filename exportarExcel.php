...
$objPHPExcel->getProperties()->setCreator("José Luis Aguilar")
							 ->setLastModifiedBy("José Luis Aguilar")
...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'REPORTE DE PARTICIPANTES')
            ->setCellValue('A2', 'CÉDULA')
            ->setCellValue('B2', 'NOMBRES')
            ->setCellValue('C2', 'TELÉFONO')
...
/*Extraer datos de MYSQL*/
    ...
    $con=@mysqli_connect('localhost', 'root', '', 'invoice_aa');
    if(!$con){
	...
	$sql="SELECT * FROM cert_participa order by nombres";
	...
	while ($row=mysqli_fetch_array($query)){
		...
		$fecha=$row['date_added'];
			...
			// Agregar datos
			$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue($a, $cedula)
            ->setCellValue($b, $nombres)
            ->setCellValue($c, $telefono)
...
$objPHPExcel->getActiveSheet()->setTitle('Reporte de participantes');
